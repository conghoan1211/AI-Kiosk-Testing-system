...
const parseRequest = (filters: IProhibitedRequest) => {
  return cloneDeep({
...
const requestAPI = prohibitedService.getAllProhibited;

const useGetListProhibited = (
  filters: IProhibitedRequest,
  options: {
    isTrigger?: boolean;
    refetchKey?: string;
    saveData?: boolean;
  ...
  const save = useSave();
  const [data, setData] = useState<ProhbitedList[]>([]);
  ...
  const checkConditionPass = useCallback(
    (response: ResponseProhibited) => {
      if (saveData) {
        ...
        save(cachedKeys.totalPageProhibitedCount, response?.data?.data.totalPage || 1);
        save(cachedKeys.totalProhibitedCount, response?.data?.data.total || 0);
        save(cachedKeys.dataProhibited, response?.data?.data?.result || []);
      }
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      if (response) {
        checkConditionPass(response as ResponseProhibited);
      }
      setRefetching(false);
    } catch (error: any) {
      if (!error.isCanceled) {
...
export default useGetListProhibited;
