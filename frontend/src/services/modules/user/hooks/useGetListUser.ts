...
import { IUserRequest, ResponseGetListUser, UserList } from '../interfaces/user.interface';
...
const parseRequest = (filters: IUserRequest) => {
  return cloneDeep({
...
const requestAPI = userService.getUserList;
...
const useGetListUser = (
  ...
  const checkConditionPass = useCallback(
    (response: ResponseGetListUser) => {
      if (saveData) {
        setTotalPage(response?.data?.data?.totalPage || 1);
        setTotal(response?.data?.data?.total || 0);
        save(cachedKeys.totalPageUserCount, response?.data?.data.totalPage || 1);
        save(cachedKeys.totalUserCount, response?.data?.data.total || 0);
        save(cachedKeys.dataUser, response?.data?.data?.result || []);
      }
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      if (response) {
        checkConditionPass(response as ResponseGetListUser);
      }
      setRefetching(false);
    } catch (error: any) {
      if (!error.isCanceled) {
