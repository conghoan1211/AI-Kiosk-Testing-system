...
const requestAPI = permissionService.getAllPermission;
...
const useGetListPermission = (
  ...
  const checkConditionPass = useCallback(
    (response: ResponsePermission) => {
      if (saveData) {
        ...
        save(cachedKeys.totalPagePermissionsListCount, response?.data?.data.totalPage || 1);
        save(cachedKeys.totalPermissionsListCount, response?.data?.data.total || 0);
        save(cachedKeys.dataPermissionsList, response?.data?.data?.result || []);
      }
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      if (response) {
        checkConditionPass(response as ResponsePermission);
      }
      setRefetching(false);
    } catch (error: any) {
      if (!error.isCanceled) {
