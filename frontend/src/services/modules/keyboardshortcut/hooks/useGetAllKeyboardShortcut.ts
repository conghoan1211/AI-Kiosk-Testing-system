...
const parseRequest = (filters: IKeyboardShorcutRequest) => {
  return cloneDeep({
...
const requestAPI = keyboardShortcutService.getAllKeyboardShortcuts;

const useGetListKeyboardShortcut = (
  filters: IKeyboardShorcutRequest,
  options: {
    isTrigger?: boolean;
    refetchKey?: string;
    saveData?: boolean;
  ...
  const save = useSave();
  const [data, setData] = useState<KeyboardShortcutList[]>([]);
  ...
  const checkConditionPass = useCallback(
    (response: KeyboardShortcutListResponse) => {
      if (saveData) {
        ...
        save(cachedKeys.totalPageKeyboardShortcutCount, response?.data?.data.totalPage || 1);
        save(cachedKeys.totalKeyboardShortcutCount, response?.data?.data.total || 0);
  ...
  const refetch = useCallback(async () => {
    try {
      ...
      if (response) {
        checkConditionPass(response as KeyboardShortcutListResponse);
      }
      setRefetching(false);
    } catch (error: any) {
      if (!error.isCanceled) {
...
export default useGetListKeyboardShortcut;
